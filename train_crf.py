...
def train_and_test(train_path, data_save_path, embedding_path, conf_path, model_save_path, model_name, summaries_dir):
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            sequence_labeler = CRFSequenceLabeler(sequence_length=max_length, embedding=embedding, cell_size=cell_rnn_size, num_classes=len(cl), hls=hidden_layer_size)
            ...
            for epoch in range(epochs):
                ...
                for batch in batches:
                    ...
                    step, loss, summary = sequence_labeler.train(sess, train_x, train_y, dropout)
                    #writer.add_summary(summary, step)
                    print("Training: epoch\t{:g}\tstep\t{:g}\tloss\t{:g}".format(epoch, step, loss))

                # Evaluate on validation and test set
                vd_loss, vd_scores, vd_accuracy, transition_params, tagged_sequences = sequence_labeler.predict(sess, valid_x, valid_y)
